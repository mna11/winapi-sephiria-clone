...
{
	HANDLE	hFile = CreateFile(L"../Data/Tile_Stage01.dat", // 이름을 포함한 파일 경로
...
{
	HANDLE	hFile = CreateFile(L"../Data/Tile_Client_Stage01.dat", // 이름을 포함한 파일 경로
		GENERIC_WRITE,		// 파일 접근 모드
		NULL,				// 공유 방식, NULL로 지정하면 공유하지 않음
		NULL,				// 보안 속성, NULL인 기본 값 설정
		CREATE_ALWAYS,		// 생성 방식, 쓰기(CREATE_ALWAYS), 읽기(OPEN_EXISTING)
...
void CTileMgr::ExtractPNG()
{
	...
	bitmap.Save(L"../Resource/Image/Stage/Stage01.png", &pngClsid, NULL);
}
