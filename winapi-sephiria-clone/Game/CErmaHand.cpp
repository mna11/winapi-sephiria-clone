...
void CErmaHand::Render(Graphics* pGraphics)
{
    ...
    if (pImg != nullptr)
    {
        constexpr float fCellX = 62.f;
        constexpr float fCellY = 64.f;
        constexpr int iFrame = 0;
...
{
    if (m_eCurState == ERMA_HAND_STATE::BROKEN || m_bHit)
        return;
...
void CErmaHand::Move()
{
    ...
    case ERMA_HAND_STATE::READY:
        if (m_pTarget != nullptr)
        {
            ...
            VEC vFollowPoint{
                m_vSlamTarget.fX,
                m_vSlamTarget.fY - FOLLOW_HEIGHT
            };
            VEC vDirection = vFollowPoint - m_tInfo.vPoint;
...
void CErmaHand::RenderAttackEffect(Graphics* pGraphics, const VEC& vScroll)
{
    ...
    if (pShadow != nullptr)
    {
        constexpr float fShadowX = 42.f;
        constexpr float fShadowY = 17.f;
        VEC vShadowSize = VEC{ fShadowX, fShadowY } * PIXEL_SCALE;
        RectF rcShadow{
    ...
    if (m_dStateElapseTime < ALERT_START_TIME)
        return;
    ...
    constexpr float fCellX = 32.f;
    constexpr float fCellY = 32.f;
    ...
    VEC vAlertSize = VEC{ fCellX, fCellY } * PIXEL_SCALE;
    RectF rcAlert{
